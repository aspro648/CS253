...
IP_URL = "http://api.hostip.info/?ip=" # 75.170.5.213
...
class Asciichan(Handler):
    ...
    def render_front(self, title="", art="", error=""):
        ...
        arts = list(arts) # saves db querries
        points = []
        for a in arts:
            if a.coords:
                points.append(a.coords)
        # or points = filter(None, (a.coords for a in arts))
        
        img_url = None
        if points:
            img_url = gmaps_img(points)

        self.render("front.html", title=title, art=art, arts=arts, img_url=img_url, error=error)
        
        
class AsciichanOrg(Handler):
    def get(self):
...
class ROT13(Handler):
    ...
    def post(self):
        ...
        self.render('ROT13.html', text=rot13, message="Hit [submit] again to unscramble!")   

 
class ROT13_mine(webapp2.RequestHandler):
...
class Signup(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if error:
            ...
            self.redirect('/hw/welcome?username=%s' % username)
...
app = webapp2.WSGIApplication([('/hw/validation', Validation), ('/hw/signup', Signup), ('hw/welcome', Welcome),
                               ('/hw/thanks', ThanksHandler), ('/hw/ROT13', ROT13), ('/hw/asciichan', Asciichan),
                               ('/hw/cookiecounter', CookieCounter), ('/hw*.', Main)],
                              debug=True)